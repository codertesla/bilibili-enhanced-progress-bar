...
Bilibili / B 站视频进度条增强油猴脚本。仅在全屏或网页全屏时显示自绘进度条（暂停时显示，可切换为永久显示），非全屏交给 B 站原生进度条。

默认使用 B 站官方蓝渲染一条低调的自绘进度条，并展示已缓冲进度。

## 功能

- 仅在全屏 / 网页全屏时显示自绘进度条；普通窗口不叠加，避免与原生进度条重复。
- 全屏下暂停视频时显示当前进度；油猴菜单可切换“永久显示进度条”，默认关闭。
- 自绘进度条包含已播放进度和已缓冲进度。
- 颜色使用 B 站官方蓝 `#00aeec`。
...
- `永久显示进度条`：默认关闭。开启后在全屏/网页全屏下，播放与暂停时都显示进度条。
